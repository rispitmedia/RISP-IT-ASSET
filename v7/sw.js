const CACHE_NAME='risp-it-asset-v7-5-2-analytics-money-pin';
const CORE=[
  './','./index.html','./styles.css?v=752','./app.js?v=752','./manifest.webmanifest?v=752',
...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith(caches.match(req).then(cached=>{const net=fetch(req).then(res=>{if(res&&res.ok){const copy=res.clone();caches.open(CACHE_NAME).then(c=>c.put(req,copy))}return res}).catch(()=>cached);return cached||net}));
});
